...
def google_login_view(request):
    ...
    try:
        ...
        if google_resp.status_code != 200:
            logger.error(
                f"Google Auth failed: {google_resp.status_code} - {google_resp.text}"
            )
            return Response(
...
def buy_tickets_view(request):
    ...
    costs = {
        "small": {"coins": 50, "tickets": 5},
        "large": {"coins": 100, "tickets": 15},
    }

    if package not in costs:
        return Response(
            {"error": "Unknown package"}, status=status.HTTP_400_BAD_REQUEST
        )

    cost = costs[package]
    if profile.coins < cost["coins"]:
        return Response(
            {"error": "Недостатньо монет"}, status=status.HTTP_400_BAD_REQUEST
        )
