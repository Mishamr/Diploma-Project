...
MAX_CONCURRENT_CATEGORIES = 2   # не більше 2 категорій одночасно (анти-бан)
MAX_CONCURRENT_PAGES = 2        # сторінок одночасно
MAX_PAGES_PER_CATEGORY = 20
REQUEST_TIMEOUT = 30
RETRY_ATTEMPTS = 2              # повторні спроби при timeout
RETRY_DELAY = 2.0               # секунд між спробами

# Ротація браузерних профілів (curl_cffi impersonate)
BROWSER_PROFILES = [
    "chrome110", "chrome107", "chrome104", "chrome101",
    "safari15_5", "safari16", "firefox110",
]
...
class ATBScraper:
    ...
    async def _run(self):
        ...
        async with AsyncSession(impersonate=random.choice(BROWSER_PROFILES)) as session:
            tasks = [
                self._scrape_category(session, semaphore, cat)
    ...
    async def _scrape_category(self, session: AsyncSession, semaphore: asyncio.Semaphore, category_path: str) -> list:
        ...
        async with semaphore:
            ...
            page1_products, has_more = await self._fetch_page(session, category_path, 1, category_name)
            ...
            if has_more:
                ...
                page_tasks = [
                    self._fetch_page_limited(session, page_sem, category_path, p, category_name)
    ...
    async def _fetch_page_limited(self, session, semaphore, category_path, page, category_name):
        async with semaphore:
            return await self._fetch_page(session, category_path, page, category_name)

    async def _fetch_page(self, session: AsyncSession, category_path: str, page: int, category_name: str):
        """Fetch and parse a single page with retry. Returns (products, has_next_page)."""
        ...
        for attempt in range(RETRY_ATTEMPTS + 1):
            try:
                ...
                logger.warning(f"[attempt {attempt + 1}/{RETRY_ATTEMPTS + 1}] {url}: {e}")
